...
export default function NewRoleManagement() {
  // Hooks e estados
  ...
  const loadRolePages = useCallback(async (roleId: string) => {
    if (loading) return;
  ...
  const handleRoleClick = useCallback(async (role: UserRole) => {
    if (selectedRole?.id === role.id) {
      // Se clicar no mesmo perfil, apenas muda para a aba de permissões
      setActiveTab("permissions");
      return;
    }
  ...
  }, [isAdmin, isLoaded, loadRoles, loadPages]);
